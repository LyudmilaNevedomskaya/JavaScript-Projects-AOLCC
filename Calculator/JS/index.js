...
function Input_Digit(digit) {
  ...
  if (Wait_Second_Operand === true) {
    ...
    // this overwrites Display_Value if the current value is 0 otherwise is adds onto it
...
function Handle_Operator(Next_Operator) {
  ...
  // when an operator key is pressed we convert the current number displayed on the screen to a number and then store the result in Calculator.First_Operand if it doesn't alrady exist
...
function Update_Display() {
  // makes use of the calculator-screen class to arget the input tag in the HTML document
...
keys.addEventListener('click', (event) => {
  // the target variablr is an object that represents the element that was clicked
  const { target } = event;
  ...
  // ensures that AC clears all inputs from the screen
